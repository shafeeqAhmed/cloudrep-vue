<template>
  <div>
    <validation-observer ref="infoRules" tag="form">
      <b-row class="align-items-center">
          ...
          <b-form-group>
            <validation-provider #default="{ errors }" name="Title" rules="required" >
              <div class="input-gradient">
                <b-form-input id="name" v-model="name" placeholder="Section Name"
                :state="errors.length > 0 ? false : null" maxLength="70"/>
              </div>
        ...
        <b-col md="7" class="text-right ml-auto">
          <b-button class="btn btn-primary" variant="primary" @click="createLesson">Create Section</b-button>
          <!-- <b-button
    ...
    <b-row>
      <b-col md="12" class="mt-3" v-if="lessons.length > 0">
        <div class="d-flex justify-content-end">
          <b-button variant="primary" class="" @click="gotoNextPage()">Next</b-button>
        </div>
      </b-col>
    </b-row>
  </div>
  ...
  methods: {
    createLesson() {
      ...
      if (this.name == '' || this.name == null) {
        this.errorToast('Error', 'Lesson name can not be empty')
        return false
      }
      ...
      this.$http
        ...
        .then(response => {
          const { data: { data: { lesson } } } = response
