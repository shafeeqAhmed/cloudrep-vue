<template>
    <div>
      <div class="">
        <div class="">
          <b-button id="popOverId" class="popover-btn">
            <feather-icon icon="FilterIcon" class="cursor-pointer" size="18" />
          </b-button>
        </div>
        <b-popover target="popOverId" custom-class="route_filter_modal">
  ...
  export default {
    ...
    methods: {
      ...
      removeAllOperatorValue(key) {
        ...
        this.conditions[key].states = [...this.$store.state.clientCampaign.states];
      },
      ...
      saveFilterRecord() {
        const campaign = this.$store.state.clientCampaign.target.target_uuid
        ...
        this.conditions.forEach((element) => {
          payload.filters.push({
            ...
            target_uuid: this.$store.state.clientCampaign.target.target_uuid,
          });
        });
  
        this.$store.dispatch("clientCampaign/storeCampaignFilterRecord", payload).then(()=>{
          this.assignValue();
        });
        // this.conditions.forEach((data) => {
        //   data.val = [];
      ...
      assignValue(){
        this.$store.dispatch("clientCampaign/getFilterRecord", this.$store.state.clientCampaign.campaignUuid).then(() => {
