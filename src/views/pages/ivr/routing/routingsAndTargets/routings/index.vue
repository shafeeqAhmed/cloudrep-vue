          ...
          <template #cell(actions)="data">
            ...
            <div class="d-flex align-items-center justify-content-center">
              <div class="action-btn">
                ...
                <feather-icon icon="TrashIcon" size="20" class="mr-1" @click="deletePlan(data.item.uuid)" />
...
export default {
  ...
  watch: {
    '$store.state.routingsAndTargets.fetchRoutingPlan': function () {
      this.refetchData()
    }
  },
  ...
  methods: {
    edit(uuid) {
      ...
      this.$store.dispatch('routingsAndTargets/getRoutingPlanDetail', uuid)
        .then((response) => {
          this.$store.commit('routingsAndTargets/UPDATE_ROUTING_PLAN', response.data.data.routingPlan)
        })
    ...
    deletePlan(uuid) {
      this.$bvModal
        ...
        .then((value) => {
          if (value) {
            this.$store
              .dispatch("routingsAndTargets/deleteRoutingPlan", { uuid })
              .then((res) => {
                this.$store.commit('routingsAndTargets/UPDATE_FETCH_TARGETS', (Math.floor(Math.random() * 10000) + 1))
                this.refetchData()
              });
          }
        });
