...
export default {
  ...
  computed: {
    getReportHistory() {
      return this.$store.getters["manageClientReporting/getTimeLine"];
    },
  },
  methods: {
    /*  getTimeChartHistory() {
      const filterState =
        this.$store.state.manageClientReporting.timeLineFilter;
  ...
  watch: {
    "$store.state.manageClientReporting.timeLineFilter": {
      handler() {
