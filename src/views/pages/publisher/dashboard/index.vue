...
export default {
  ...
  computed: {
    filterRecord() {
      return this.$store.getters["publisherDashboard/getDashboardFilterRecord"];
    },
  },
  methods: {
    getStats() {
      this.isFilterDisable = 1;
      this.$store.dispatch("publisherDashboard/getDashboardFilterRecord", {
        page: "1",
      });
      this.$store
        .dispatch("publisherDashboard/getUserDashboardRecord", {
        ...
        .then((data) => {
          const {
            ...
            total_goal,
            total_incomming,
          } = this.$store.getters["publisherDashboard/getStatistics"];
          ...
          this.dashboardData.statisticsItems[3]["title"] = total_payout;
  ...
  created() {
    const APP_STORE_MODULE_NAME = "publisherDashboard";
