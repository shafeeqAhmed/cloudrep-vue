<template>
  <div>
    <div class="header_title">
      <b-row class="align-items-center">
        <b-col lg="8" md="6" sm="12" cols="12">
          <h2>LMS > Drafted Course List</h2>
          <p>Enter your personal information brfore you can start using this platform.</p>
        </b-col>
    ...
    <div class="lms-courselist mb-5" v-if="courseList">
      <b-row class="kb-search-content-info match-height" >
        <b-col sm="12"  v-for="course in courseList" :key="course.id">
          <div class="uni-course-card">
            <div class="course-card-img"  @click="courseView(course.lms_course_uuid)">
              <div class="img-overlay"></div>
              <b-img :src="course.course_image"></b-img>
            </div>
            <div class="course-card-content">
                <h2  @click="courseView(course.lms_course_uuid)">{{ course.title }}</h2>
                <p style="display: -webkit-box; overflow: hidden; -webkit-line-clamp: 3; -webkit-box-orient: vertical;">
                  {{ course.tag_line }}
                </p>
                ...
                <div class="courselist-edit" v-b-toggle="`courseEdit-${course.id}`">
                  <feather-icon icon="MoreVerticalIcon" size="21" />
                </div>
                <b-collapse :id="`courseEdit-${course.id}`" class="courseEdit" >
                  <div class="course-edit-box">
                    <b-button class="courseedit-btn mb-1" @click="editCourse(course.lms_course_uuid)">Edit</b-button>
                    <b-button class="coursedelete-btn" @click="deleteCourse(course.lms_course_uuid)">Delete</b-button>
                  </div>
                </b-collapse>
              </div>
          </div>
        ...
        <b-col cols="12" class="mt-1 pb-5">
          <b-pagination v-model="currentPage" :total-rows="totalRows" :per-page="perPage" align="center" size="sm"
            class="my-0" />
        </b-col>
      </b-row>
    </div>
    <div v-else class="lms-listing-none mt-4 mb-5 d-flex align-items-center justify-content-center">
...
export default {
  ...
  methods: {
    editCourse(param){
      this.$router.push({ name: 'create-course', params: { course_uuid: param } })
    },
    getCourse() {
      this.showLoader();
      this.$http
        .get("/get-course-list-drafted", {
          params: {
            q: this.searchQuery,
