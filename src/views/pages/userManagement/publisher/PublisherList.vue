        ...
        >
          ...
          <template #cell(actions)="data">
            ...
            >
              ...
              <b-dropdown-item
                v-if="data.item.step == 6"
                @click="loginAs(data.item.user_uuid)"
              >
                <feather-icon
                  icon="UserIcon"
              ...
              <b-dropdown-item
                v-if="data.item.can_create_a_number == 1"
                @click="isAllowed(data.item.user_uuid, 'false')"
              >
                <feather-icon
                  icon="UserIcon"
                  class="mr-50 text-success"
                />
                <span class="text-success">Un allow number</span>
              </b-dropdown-item>
              <b-dropdown-item
                v-else
                @click="isAllowed(data.item.user_uuid, 'true')"
              >
                <feather-icon
                  icon="UserIcon"
                  class="mr-50 text-success"
                />
                <span class="text-success">Allow number</span>
              </b-dropdown-item>
            </b-dropdown>
          </template>
        </b-table>
...
export default {
  ...
  methods: {
    ...
    isAllowed(uuid, isActive) {
      const params = {
        uuid: uuid,
        can_create_a_number: isActive,
      };
      this.$http
        .post("update-number", params)
        .then((response) => {
          this.refetchData();
          // this.onChangeRefetch = Math.floor(Math.random() * 10)
          if (isActive == true) {
            this.successToast(
              "Success",
              "Number allowed to this publisher successfully!"
            );
          }
          if (isActive == false) {
            this.successToast(
              "Success",
              "Number allowed removed from this publisher successfully!"
            );
          }
        })
        .then((error) => {});
