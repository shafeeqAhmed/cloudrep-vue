<template>
  <div>
    ...
    <b-row class="mb-2">
      ...
      <b-col cols="6" class="">
        <b-button variant="primary" class="float-right" v-b-modal.store-modal>+ Create Routing Plan</b-button>
      </b-col>
    </b-row>
        ...
        <b-table ref="refListTable" responsive :per-page="perPage" :current-page="currentPage" :items="fetchRecords"
          ...
          <!-- <template #cell(is_active)="data">
            <div class="active-num d-flex align-items-center justify-content-center">
              <div v-if="data.item.is_active == 1" class="active-num d-flex align-items-center">
                Active
              </div>
          ...
          <template #cell(actions)="data">
            ...
            <div class="d-flex align-items-center justify-content-center">
              <div class="action-btn">
                <feather-icon icon="EditIcon" size="20" class="cursor-pointer mr-1" @click="edit(data.item)" />
                <feather-icon icon="PlusIcon" size="20" class=" cursor-pointer mr-1"
                  @click="$router.push({ name: 'routings-and-targets', params: { uuid: data.item.uuid } })" />
                <feather-icon icon="TrashIcon" size="20" class="cursor-pointer" @click="deleteRecord(data.item.uuid)" />
              </div>
            </div>
    ...
    <store-routing-plan-modal :editRecord="editRecord" :refetchData="refetchData" />
...
import StoreRoutingPlanModal from './components/StoreRoutingPlanModal.vue'
...
export default {
  ...
  methods: {
    ...
    deleteRecord(uuid) {
      ...
      this.$bvModal
        .msgBoxConfirm('Are you Really want to delete this record', {
        ...
        .then((value) => {
          if (value) {
            this.$store.dispatch('routingPlans/deleteRouting', { uuid }).then(() => {
              this.refetchData()
            })
          }
        });
  ...
  setup() {
    const APP_STORE_MODULE_NAME = "routingPlans";
