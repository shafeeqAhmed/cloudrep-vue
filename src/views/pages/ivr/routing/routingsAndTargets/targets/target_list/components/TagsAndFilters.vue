...
export default {
  ...
  methods: {
    ...
    assignAllOperatorValue(key) {
      const list = this.$store.state.routingsAndTargets.states.map((data) => {
        return data.name;
      });
    ...
    removeAllOperatorValue(key) {
      ...
      this.conditions[key].states = [
        ...this.$store.state.routingsAndTargets.states,
      ];
    },
    ...
    conditionTags() {
      if (this.$store.state.routingsAndTargets.tags.length == 0) {
        this.$store.dispatch("routingsAndTargets/getTags").then(() => {
          this.tags = this.$store.state.routingsAndTargets.tags;
        });
      } else {
        this.tags = this.$store.state.routingsAndTargets.tags;
      }
    ...
    getTagFilters(value, key) {
      ...
      this.$store
        .dispatch("routingsAndTargets/getTagFilters", value)
        .then(() => {
          this.conditions[key].operators =
            this.$store.state.routingsAndTargets.tagsFilters;
        });

      const tag = this.$store.state.routingsAndTargets.tags.find(
        (data) => data.uuid === value
      );

      if (tag.value === "state") {
        if (this.$store.state.routingsAndTargets.states.length == 0) {
          this.$store.dispatch("routingsAndTargets/getStates").then(() => {
            this.conditions[key].states = [
              ...this.$store.state.routingsAndTargets.states,
            ];
    ...
    assignValue() {
      this.$store
        .dispatch("routingsAndTargets/getFilterRecord", this.target_uuid)
        .then(() => {
          const filterRecord =
            this.$store.state.routingsAndTargets.filterRecord;
