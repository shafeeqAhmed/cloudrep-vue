<template>
  <div>
    <Container @drop="onDrop">
      <Draggable
        class="router_tab mb-1"
        v-for="(filter, key) in node.filters"
        :key="key"
      >
...
export default {
  ...
  methods: {
    ...
    onDrop(dropResult) {
      ...
      this.$store
        .dispatch("ivrBuilder/reorderRouterFilters", { filters: filterOrder })
        .then(() => {
          ...
          this.$store.dispatch("ivrBuilder/getIvr", payLoad);
        });
